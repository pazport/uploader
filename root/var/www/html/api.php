...
foreach (new DirectoryIterator('/config/json/') as $json) {
    if ($json->isFile() && !$json->isDot()) {
        ...
        if ($file->filebase == null) {
            continue;
        }
        $jsonArray[$file->status][$file->filebase] = [
            'GDSA'    => $file->gdsa,
        ...
        if ($file->status == 'uploading') {
            ...
            if ($matches) {
                $jsonArray[$file->status][$file->filebase]['upload'] = ['percent'=> $matches[1], 'rate'=>$matches[2], 'time'=>$matches[3]];
            } else {
                $jsonArray[$file->status][$file->filebase]['upload'] = ['percent'=>'100%', 'rate'=>'0MB/s', 'time'=>'0s'];
            }
        }
        if ($file->status == 'done') {
